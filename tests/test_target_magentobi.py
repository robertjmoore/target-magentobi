...
class DummyClient(target_magentobi.DryRunClient):
...
def persist_all(recs):
    with DummyClient() as client:
        target_magentobi.persist_lines(client, message_lines(recs))
...
class TestTargetMagentoBI(unittest.TestCase):
    ...
    def test_persist_lines_sets_key_names(self):
        ...
        with DummyClient() as client:
            target_magentobi.persist_lines(client, message_lines(inputs))
    ...
    def test_persist_last_state_when_stream_ends_with_record(self):
        ...
        with mock.patch('sys.stdout', buf):
            with DummyClient(buffer_size=3) as client:
                ...
                final_state = target_magentobi.persist_lines(client, message_lines(inputs))
